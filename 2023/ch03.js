...
function parseItem (item = '') {
  ...
  const keySplit = keyValue[0].split(' ');
  const minMax = keySplit[0].split('-');
  return {
    min: +minMax[0],
    max: +minMax[1],
...
function getInvalidPasswords (input = '') {
  const inputArr = input.split('\n');
  const invalidPasswords = [];
  
  inputArr.forEach(item => {
    const validations = parseItem(item);
    if (!validatePassword(validations)) {
      invalidPasswords.push(validations.value);
    }
  });
  return invalidPasswords;
}

function validatePassword (inst) {
  let isValid = true;
  const regex = new RegExp(`[^${inst.key}]`, 'g');
  const keyOccurrences = inst.value.replace(regex,'')?.length;
  if (keyOccurrences <= 0) return false;
  
  if (keyOccurrences < inst.min && keyOccurrences <= inst.max) {
    isValid = false;
  }
  
  return isValid;
}
console.log(getInvalidPasswords(input)[41]);